            ...
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6" style="text-align: right">
                        <a href=" {{ url('employee/forum/posts/create') }} " class="btn btn-primary"> Create Posts</a>
                    </div><!-- /.col -->
                </div><!-- /.row -->
            </div><!-- /.container-fluid -->
        </div>
                            ...
                            <div class="card mb-3">
                                ...
                                <div class="card-body">
                                    ...
                                    @if($data->employee_id == Auth::user()->id)
                                        <a style="margin-left:10px;" class="btn btn-danger float-right" onclick="return confirm('Are you sure want to delete?')" href="{{ url('employee/forum/delete/'.$data->id) }}">Delete</a>
                                    @else
                                        <a style="margin-left:10px;" class="btn btn-danger float-right" onclick="alert('You are not allow to delete')">Delete</a>
                                    @endif

                                    @if($data->employee_id == Auth::user()->id)
                                        <a style="margin-left:10px;" class="btn btn-primary float-right" href="{{ url('employee/forum/edit/'.$data->id) }}">Edit</a>
                                    @else
                                        <a style="margin-left:10px;" class="btn btn-primary float-right" onclick="(alert('You are not allow to edit'))">Edit</a>
                                    @endif

                                    <a class="btn btn-primary float-right" href="{{ url('employee/forum/view/'.$data->id) }}">View</a>
